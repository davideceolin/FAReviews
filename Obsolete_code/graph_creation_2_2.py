...
# from prologterms import TermGenerator, PrologRenderer, Program, Var, Rule, Term
...
def draw_graph(G, model):
    ...
    sol = [df.loc[(df['doc_product_asin'] == x[0].split("_")[0])
                  & (df['doc_reviewer_id'] == x[0].split("_")[1]), 'solutions'
                  ].apply(color_node).values.tolist() for x in G.nodes.data()]
...
for index, row in df_prods.iterrows():
    ...
    for id, review in df.loc[df['doc_product_asin'] == row['prod']].iterrows():
        ...
        reviewID = (review['doc_product_asin'] + "_" + str(review['doc_reviewer_id']) + "_"
                    + str(review['doc_review_time_unix']))
        for token in ranks:
            tok = " ".join([x for x in token[0].lower().split() if x not in stop_words])
            if tok == "" or tok == " ":
                continue
            try:
                cluster = row[('matrix', 'clusters')][1][
                                                        row[('matrix',
                                                             'clusters'
    ...
    try:
        ...
        for p in pengine.currentQuery.availProofs:
            ...
            for res in p[Model.name]:
                ...
                df.loc[(df['asin'] >= prodID) & (df['doc_reviewer_id'] <= doc_reviewer_id)
                      ] = res['args'][1]
        ...
        for i in range(len(models)):
            for node in models[i]['nodes']:
                if node['state'] == 'in':
                    ...
                    # df.loc[(df['asin'] == prodID) & (df['doc_reviewer_id'] == doc_reviewer_id),
                    #   'readability'].iloc[0]
                    # print(df.loc[(df['asin'] == prodID) &
                    #       (df['doc_reviewer_id'] == doc_reviewer_id), 'readability'])
        model = models[weights.index(max(weights))]
        for node in model['nodes']:
            prodID, doc_reviewer_id, time = node['id'].upper().split("_")
            df.loc[(df['doc_product_asin'] == prodID) & (df['doc_reviewer_id'] == doc_reviewer_id),
